...
def get_chebyshev_scalarization(
    ...
    """
    if weights.shape[-1:] != Y.shape[-1:]:
        raise BotorchTensorDimensionError(
            "weights must be an `... x m`-dim tensor where Y is `... x m`."
    ...
    if len(weights.shape) > 2:
        raise BotorchTensorDimensionError(
            "weights must be an `k x m`-dim tensor"
            f"Got shapes {weights.shape}."
        )
    ...
    def chebyshev_obj(Y: Tensor, X: Optional[Tensor] = None) -> Tensor:
        ...
        weights_expanded = torch.atleast_2d(weights)#(num_params, num_output_dims)
        if len(Y.shape)==2:
            Y = Y.unsqueeze(dim=-2) # (num_Y_vals, 1, num_output_dims)

        product = weights_expanded * Y
