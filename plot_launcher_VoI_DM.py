...
def run_experiment(
    ...
    schedule = range(1, 20)
    for it in range(5):
        optimizer.x_recommended = optimizer.policy()
        pairs_acquisition_function = optimizer.recommenderfun(model=optimizer.model,
                                                              utility_model=optimizer.utility_model,
                                                              weights=optimizer.weights,
                                                              bounds=bounds_normalized,
                                                              Y_sampled=optimizer.utility_normalizing_vectors_true_fun,
                                                              current_best_value=optimizer.x_recommended,
                                                              optional=optimizer.optional)
        unsampled_pairs_idx_lst, unsampled_pairs_lst = optimizer.sample_filtering()
        KG_vals_DM = pairs_acquisition_function.forward(pairs=unsampled_pairs_lst)
        best_pair_idx, best_pair, voi_dm = optimizer.optimize_sampled_pairs_fun(acq_fun=pairs_acquisition_function)
        if it in schedule:
            optimizer.elicit_info_from_dm(pair_new_idx=best_pair_idx, y_1=best_pair[0], y_2=best_pair[1])
            optimizer._update_preference_model()

        output = {"pairs": unsampled_pairs_lst,
                  "acqvals": KG_vals_DM,
                  "y":optimizer.y_train,
                  "posterior_samples": optimizer.weights,
                  "best_pair": best_pair,
                  "parameters": optimizer.weights,
                  "iteration": it}

        savefile = "/home/jungredda/Documents/GITHUB_REPOS/Preferential_BICO/data_for_plots"
        if os.path.isdir(savefile) == False:
            os.makedirs(savefile, exist_ok=True)

        with open(savefile + "/iter_" + str(it) + ".pkl", "wb") as f:
            pkl.dump(output, f)
